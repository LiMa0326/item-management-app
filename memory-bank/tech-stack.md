...
- V0 发布前 UI 统一规范（新增）：`Category` 作为根入口；统一 `TopAppBar + Overflow` 骨架；页面级 `Refresh` 进入 Overflow；非编辑页移除底部 Back；支持 Category->ItemList 预筛选路由参数。
- UI 组件策略（新增）：页面外壳统一抽象为可复用 Scaffold（标题/返回/菜单动作），页面内仅维护业务内容区，避免每页重复实现导航与操作栏。
...
- 备份导出/导入实现思路（严格对齐 `memory-bank/BACKUP_FORMAT.md`）：
  ...
  - UI 重构约束：仅调整视觉结构与交互入口，不变更备份包结构、字段与导入导出语义。
